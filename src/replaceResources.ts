...
import {
  ...
  filesPathSetting,
  syncConfigured,
  runOnStartAndAfterSyncSetting,
  getResourceById,
} from "./replaceResourcesApi";
...
const regExpFileResourceReplace: RegExp =
  /^.*~[a-zA-Z0-9]{32}~.*~[0-9]{13}.REPLACE$/;
const fileSeparator = "~";
  ...
  if (baseDir?.length > 0) {
    ...
    for (const fullNameExt of allFiles) {
      ...
      if (filename != step1DirName && filename != step2DirName) {
        try {
          if (regExpResourceId.test(filename)) {
            console.debug(
              `##DEBUG: deleteResources: filename IS a ResourceId so we will use that to search: ${filename}`
            );
            resourceSearchString = filename;
            originalResource = await getResourceById(resourceSearchString);
            if (originalResource) {
              console.info(
                `Resource found with id: ${resourceSearchString}. Proceeding.`
              );
              ...
              console.info(
                `No resource found with id: ${resourceSearchString}. Not proceeding.`
              );
            }
          } else {
            console.debug(
              `##DEBUG: deleteResources: filename NOT a ResourceId, will use fullNameExt to search : ${fullNameExt}`
            );
            resourceSearchString = fullNameExt;
            originalResource = await getResourceByFilename(
              resourceSearchString
            );

            if (originalResource.items.length == 1) {
              console.debug(
                `##DEBUG: deleteResources: originalResource.items.length = 1: ${originalResource.items.length}`
              );
              resourceId = originalResource.items[0].id;
              resourceTitle = originalResource.items[0].title;
              resourceUserCreatedTime =
                originalResource.items[0].user_created_time;
              console.info(
                `Resource found with search: ${resourceSearchString}. Its Resource Id is: ${resourceId}`
              );
              console.debug(
                `##DEBUG: deleteResources: resourceTitle: ${resourceTitle} & resourceUserCreatedTime: ${resourceUserCreatedTime}`
              );
              ...
              console.info(
                `More than one resource found with search: ${resourceSearchString}. Not proceeding.`
              );
            } else {
              console.info(
                `No resource found with search: ${resourceSearchString}. Not proceeding.`
              );
            }
          }
        } catch (error) {
          console.error(
            `ERROR - GET Resource by search: ${resourceSearchString} ${error}`
          );
        }
      }
      ...
      if (resourceId && resourceUserCreatedTime) {
        console.debug(
          `##DEBUG: deleteResources: resourceId ${resourceId}, resourceTitle ${resourceTitle} & resourceUserCreatedTime ${resourceUserCreatedTime} all have values, proceed with delete`
        );
        let fileResourceReplace = generateReplaceFileName(
          filename,
          fileExt,
          resourceId,
          resourceTitle,
        ...
        console.debug(
          `##DEBUG: deleteResources: fileResourceReplace: ${fileResourceReplace}`
        );
        ...
        try {
          ...
          console.info(
            `.replace audit file created: ${fileResourceReplacePath}`
          );
          ...
          try {
            ...
            try {
              ...
              let step1DirAndFileReplace = path.join(
                step1Dir,
                fileResourceReplace
              );
              console.debug(
                `fileResourceReplacePath ${fileResourceReplacePath}`
              );
              console.debug(`step1DirAndFileReplace ${step1DirAndFileReplace}`);
              let fileMoveReplace = await fs.move(
                fileResourceReplacePath,
                step1DirAndFileReplace,
                { overwrite: true }
              );
              ...
              console.error(
                `ERROR - moving files to step 1 directory: ${error}`
              );
            }
          } catch (error) {
            console.error(
              `ERROR - DELETE Resource: ${resourceId} / ${fullNameExt} ${error}`
            );
          }
        } catch (error) {
          console.error(
            `ERROR - creating .REPLACE file ${fileResourceReplace}. ${error}`
          );
        }
      } else {
        console.debug(
          `##DEBUG: NOT PROCEEDING with delete: deleteResources: resourceId: ${resourceId}, resourceTitle: ${resourceTitle}, resourceUserCreatedTime: ${resourceUserCreatedTime}`
        );
      }
    }
    ...
    if (createResourcesProceed) {
      console.debug(
        `##DEBUG: deleteResources: createResourcesProceed: ${createResourcesProceed}`
      );
      ...
      if (isSyncConfigured) {
        ...
        console.debug(
          `##DEBUG: No need to wait for sync, going straight to createResources`
        );
  ...
  if (baseDir?.length > 0) {
    ...
    for (const fullNameExtReplace of allStep1Files) {
      if (regExpFileResourceReplace.test(fullNameExtReplace)) {
        console.debug(
          `##DEBUG: createResources: fullNameExtReplace is a REPLACE file: ${fullNameExtReplace}`
        );
        ...
        console.debug(
          `##DEBUG: createResources: fullNameSplit: ${fullNameSplit}`
        );
        ...
        try {
          console.debug(
            `##DEBUG: about to postResource: ${resourceTitle} with resourceId ${resourceId}`
          );
          let post = await postResource(
            resourceId,
            step1DirAndFile,
            resourceTitle
          );
          console.debug(
            `##DEBUG: about to putResource: ${resourceId} with UserCreatedTime ${resourceUserCreatedTime}`
          );
          ...
          console.info(
            `Attachment replaced, file moved with id: ${resourceId}`
          );
        } catch (error) {
          console.error(
            `ERROR - with either POST and PUT Resource: file: ${resourceTitle} with resource id: ${resourceId} ${error}`
          );
        }
      }
    }
    ...
    console.debug(
      `##DEBUG: isRunOnStartAndAfterSync: ${isRunOnStartAndAfterSync}`
    );
...
export function generateReplaceFileName(
  filename: string,
  fileExt: string,
  resourceId: string,
  resourceTitle: string,
  resourceUserCreatedTime: any
) {
  return (
    [
      filename + fileExt,
      ...
      resourceTitle,
      resourceUserCreatedTime,
    ].join(fileSeparator) + fileExtReplace
  );
}
